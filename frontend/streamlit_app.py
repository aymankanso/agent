...
def _perform_model_initialization_in_container(model_info):
    """Perform model initialization (directly inside container)"""
    try:
        with st.spinner(f"Initializing {model_info.get('display_name', 'Model')}..."):
            ...
            success = loop.run_until_complete(executor_manager.initialize_with_model(model_info))
