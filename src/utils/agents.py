...
class AgentManager:
    ...
    def _load_config(cls):
        ...
        if cls._config is None:
            # Find configuration file path from project root
    ...
    def normalize_agent_name(cls, agent_name: str) -> str:
        ...
        Guarantees same normalization result across all platforms
        """
        ...
        # Unified matching logic (in priority order)
        if "planner" in agent_name_lower:
            ...
            return "initial_access"  # Unified: as initial_access
            ...
            return "privilege_escalation"  # Unified: as privilege_escalation
        elif "defense_evasion" in agent_name_lower or "defense" in agent_name_lower or "evasion" in agent_name_lower:
            return "defense_evasion"  # Unified: as defense_evasion
    ...
    def get_cli_color(cls, agent_name: str) -> str:
        """Return CLI color (Rich color name)"""
    ...
    def get_frontend_color(cls, agent_name: str) -> str:
        """Return Frontend color (Hex code)"""
        config = cls._load_config()
        normalized = cls.normalize_agent_name(agent_name)
    ...
    def get_agent_info(cls, agent_name: str) -> Dict[str, str]:
        """Return all agent information at once"""
        return {
    ...
    def list_all_agents(cls) -> Dict[str, Dict[str, str]]:
        """Return list of all agent information defined in config file"""
        config = cls._load_config()
        agents = {}
        
        # Build agent list based on CLI color keys
        for agent_key in config["colors"]["cli"].keys():
            if agent_key != "default":  # Exclude default
    ...
    def get_config_path(cls) -> Optional[str]:
        """Return currently used configuration file path"""
        cls._load_config()  # Load config to set path
