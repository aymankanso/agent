...
class Executor:
    ...
    async def initialize_swarm(self, model_info: Optional[Dict[str, Any]] = None, thread_config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            # Handle thread config - Use if provided externally, otherwise create new
            if thread_config:
                self._config = thread_config
                self._thread_id = thread_config["configurable"]["thread_id"]
            else:
                # Existing method: Generate new UUID
                self._thread_id = str(uuid.uuid4())
                self._config = {
                    "configurable": {
                        "thread_id": self._thread_id,
                    },
                    "recursion_limit": 100,  # Limit total graph iterations to prevent infinite loops
                }
    ...
    async def execute_workflow(self, user_input: str, config: Optional[Dict[str, Any]] = None) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        # Use provided config, otherwise use default config
        ...
        try:
            ...
            async for namespace, output in self._swarm.astream(
                inputs,
                stream_mode="updates",
                config=execution_config,  # Use updated config
                subgraphs=True
            ):
                step_count += 1
                ...
                for node, value in output.items():
                    ...
                    if "messages" in value and value["messages"]:
                        ...
                        if messages:
                            ...
                            if should_display:
                                # Create event in format that frontend can process
                                event_data = {
                                    "type": "message",
                                ...
                                # Additional info for tool messages
    ...
    def _should_display_message(self, message, agent_name: str, step_count: int):
        """Determine whether to display message - Completely identical to CLI logic"""
        # Generate message ID
        message_id = None
        ...
        # Display user messages only once
        ...
        # Display only new tool messages
    ...
    async def change_model(self, model_info: Dict[str, Any]):
        ...
        try:
            ...
            # Regenerate agents with new model
            self._swarm = await create_dynamic_swarm()
    ...
    def get_state_dict(self):
        """Return state as dictionary (for session storage)"""
        return {
